...
export const MessageForm = ({ projectId }: Props) => {
  ...
  const createMessage = useMutation(
    trpc.messages.create.mutationOptions({
      onSuccess: () => {
        form.reset({
          value: "",
          projectId // Keep the projectId when resetting
        });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    console.log("Submitting message with values:", values);
    await createMessage.mutateAsync({
      value: values.value,
      projectId: values.projectId, // Use projectId from form values
    });
  };
            ...
            <TextareaAutosize
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
                  console.log("values: ", field.value, projectId);
                  e.preventDefault();
